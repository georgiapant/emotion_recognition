...
from src.models import BERT_bilstm
...
def get_weighted_loss(logits, labels_true, weights):
    ...
    loss_fn = nn.CrossEntropyLoss()
    weighted_loss = loss_fn(logits, labels_true)
...
def train(model, train_dataloader, val_dataloader=None, optimizer=None, scheduler=None, epochs=4, evaluation=False, weights=None):
    """
    ...
    for epoch_i in range(epochs):
        ...
        if evaluation == True:
            # After the completion of each training epoch, measure the model's performance
            # on our validation set.
            val_loss, val_accuracy = validation(model, val_dataloader, weights)
...
def bert_predict(X_test, y_test=None):
    ...
    model = torch.jit.load(project_root_path + '/models/model_scripted_multiclass_emotion_weight_oversample.pt')
    model.eval()
...
def main():
    X_train, y_train, X_val, y_val, X_test, y_test, weights = ekman_dataset(RANDOM_SEED, oversampling=False)
    ...
    train(bert_classifier, train_dataloader, val_dataloader, optimizer, scheduler, epochs=EPOCHS, evaluation=True, weights=weights)

    for batch in train_dataloader:
        b_input_ids, b_attn_mask = tuple(t.to(device) for t in batch)[:2]
        model_scripted = torch.jit.trace(bert_classifier, (b_input_ids, b_attn_mask))
        torch.jit.save(model_scripted, project_root_path+'/models/model_scripted_multiclass_emotion_weight_oversample.pt')
        break
