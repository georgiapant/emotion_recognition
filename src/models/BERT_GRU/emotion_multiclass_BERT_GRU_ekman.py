...
from src.features.preprocess_feature_creation import nrc_feats, vad_feats, create_dataloaders_BERT_w_token_type
...
from src.models import BERTGru
...
tokenizer.save_pretrained("../../models/tokenizer_gru/")
...
def initialize_model(train_dataloader, epochs=EPOCHS):
    ...
    bert_classifier = BERTGru.bertGruClassifier(freeze_bert=False)
...
def train(model, train_dataloader, val_dataloader=None, optimizer=None, scheduler=None, epochs=4, evaluation=False):
    ...
    torch.save(model.state_dict(),  project_root_path+"/models/multiclass_emotion_gru.pt")

    print("Training complete!", flush=True)


def bert_predict(model, X_test, y_test=None):
    """
    ...
    model = torch.load(project_root_path+'/models/multiclass_emotion_gru.pt')
    # model = torch.jit.load(project_root_path + '/models/model_scripted_multiclass_emotion.pt')
    model.eval()

    tokenizer = BertTokenizer.from_pretrained(project_root_path+"/models/tokenizer_gru/")
    test_dataloader = create_dataloaders_BERT_w_token_type(X_test, y_test, tokenizer, MAX_LEN, BATCH_SIZE, sampler='sequential')
    all_logits = []

    # For each batch in our test set...
    for batch in test_dataloader:
        # Load batch to GPU
        b_input_ids, b_token_type_ids, b_attn_mask = tuple(t.to(device) for t in batch)[:3]
...
def main():
    ...
    train_dataloader = create_dataloaders_BERT_w_token_type(X_train, y_train, tokenizer, MAX_LEN, BATCH_SIZE, sampler='random')
    val_dataloader = create_dataloaders_BERT_w_token_type(X_val, y_val, tokenizer, MAX_LEN, BATCH_SIZE, sampler='sequential')
    ...
    #     b_input_ids, b_token_type_ids, b_attn_mask = tuple(t.to(device) for t in batch)[:3]
    #     model_scripted = torch.jit.trace(bert_classifier, (b_input_ids,b_token_type_ids, b_attn_mask))
    #     torch.jit.save(model_scripted, project_root_path+'/models/model_scripted_multiclass_emotion_BERT_GRU.pt')
    #     break

    print("Validation set", flush=True)
    probs_val = bert_predict(bert_classifier, X_val)
    evaluate(probs_val, y_val)

    print("Test set", flush=True)
    probs_test = bert_predict(bert_classifier, X_test)
