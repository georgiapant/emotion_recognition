...
def initialize_model(train_dataloader, epochs=EPOCHS):
    ...
    classifier = BERT_NRC_VAD.bertNrcVadClassifier(freeze_bert=False)
...
def validation(model, val_dataloader):
    ...
    for batch in val_dataloader:
        ...
        vad = vad_feats(b_input_ids, tokenizer, MAX_LEN).to(device)
...
def bert_predict(X_test, y_test=None):
    ...
    model = torch.jit.load(project_root_path + '/models/model_scripted_multiclass_emotion_VAD_NRC.pt')
    model.eval()

    tokenizer = BertTokenizer.from_pretrained(project_root_path+"/models/tokenizer/")
...
def main():
    ...
    for batch in train_dataloader:
        ...
        torch.jit.save(model_scripted, project_root_path+'/models/model_scripted_multiclass_emotion_VAD_NRC.pt')
        break
